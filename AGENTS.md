...
**This app should be as lightweight and efficient as possible.** That decides
almost every question below: every dependency, every abstraction, every MB has
to earn its place.
...
**Never compile a release build on the target machine.** `lto = true` will
thrash swap on modest hardware. Build elsewhere, install the `.deb`.
...
**Don't reject a library for its footprint** without checking whether it moves
the number. The webview dominates memory by two orders of magnitude; a 5 KB
bundle versus a 60 KB one is noise. Reject for maintenance cost, not bytes.
...
src/                       frontend (TypeScript + Preact)
  ...
  bindings.ts              the IPC contract — generated, DO NOT EDIT BY HAND
  types.ts                 re-exports bindings.ts, plus the frontend-only types
...
src-tauri/src/
  ...
  bindings_test.rs         checks src/bindings.ts against the structs below
...
**`src/bindings.ts` is generated by `ts-rs` and cannot drift from Rust.**
Every struct and enum crossing the IPC boundary derives `TS` under
`#[cfg(test)]` (`ts-rs` is a dev-dependency, so this costs nothing in a
release build); `cargo test` fails if the committed file is stale. Changed a
struct? Run `UPDATE_BINDINGS=1 cargo test bindings` from `src-tauri/` and
commit the diff — do not hand-edit `bindings.ts`. `src/types.ts` re-exports it
alongside the handful of types with no Rust source (`Settings`,
`StatusSettings`, `Tone`, `BadgeSpec`).
