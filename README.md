...
**This app should be as lightweight and efficient as possible.** That
principle drives most of the design decisions below.
...
| | |
|---|---|
| **Shell** | Tauri 2 — uses the WebKitGTK already on the system instead of shipping a second browser. Electron idles around 400 MB; this sits near 100–150 MB. |
...
src/                     frontend (TypeScript + Preact)
  bindings.ts            the IPC contract — generated by ts-rs from Rust
  types.ts               re-exports bindings.ts, plus frontend-only types
...
Rust 1.85+ and the Tauri system libraries. On Debian-based distributions:
